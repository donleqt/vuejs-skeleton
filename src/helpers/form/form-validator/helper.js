      ...
      case 'INPUT':
        switch (form.elements[i].type) {
          ...
          case 'radio':
            ...
            break;
          case 'file':
            const { files } = form.elements[i];
            if (files && files[0]) {
              q.push([form.elements[i].name, form.elements[i].files[0]]);
            }
            break;
        }
        break;
...
export const validate = (formData, formSchema) => {
  // fill label
  objectMap(formSchema, (e, name) => {
    if(!e._label) {
      e._label = name.toCapitalize();
    }
  });
